...
const counter = {};
//reset counter for each city to 0
northAmerica.forEach((city) => {
  counter[`${city}`] = 0;
});
asia.forEach((city) => {
  counter[`${city}`] = 0;
...
southAmerica.forEach((city) => {
  counter[`${city}`] = 0;
});
antarctica.forEach((city) => {
  counter[`${city}`] = 0;
});
africa.forEach((city) => {
  counter[`${city}`] = 0;
...
const reNameCities = (data) => {
  let cityName = data.attributes[14].value;
  counter[`${cityName}`] += 1;
  let cityNameNumber = cityName + " #" + counter[`${cityName}`];
  let fileName = cityNameNumber + ".png";
  data.name = cityNameNumber;
...
let file1Raw = fs.readFileSync("continentmetadata.json");
...
fs.writeFile("renamemetadata.json", writeData, (err) => {
  if (err) throw err;
